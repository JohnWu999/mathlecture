...
export function getProjectTypeLabel(projectType = "PAID") {
  return PROJECT_TYPE_COPY[projectType] || PROJECT_TYPE_COPY.PAID;
}
...
export function getProjectReadiness(project = {}) {
  ...
  return {
    ...
    tags,
    helper: "这里说的是参与这个项目前最好具备的经验，不是公开积分门槛。",
  };
}
...
export function getProjectEnrollmentCopy({ projectType = "PAID", price = 0 } = {}) {
  if (projectType === "FREE" || price === 0) {
    return {
      cta: "申请加入项目",
      priceLabel: "免费体验",
      helper: "提交后由老师确认是否适合本期节奏，再安排进入任务或小组。",
    };
  }
  return {
    cta: "提交报名意向",
    priceLabel: `主题服务 ¥${Math.round(price / 100)}`,
    helper: "先留下报名意向，老师或管理员人工确认服务内容、时间与名额；MVP 阶段会先确认是否适合本期项目。",
  };
}
