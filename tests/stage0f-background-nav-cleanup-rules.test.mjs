...
test('phone navbar stacks the brand vertically and gives page titles their own row to prevent overlap', () => {
  ...
  assert.match(phoneBlock, /\.forest-brand\s+:global\(\.brand-logo-mark\)\s*\{[^}]*width:\s*14px[^}]*height:\s*9px/, 'phone Logo should be smaller than previous 18x12 size and target the child SVG globally');
