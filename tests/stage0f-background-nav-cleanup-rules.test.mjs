...
test('phone navbar stacks the brand vertically and gives page titles their own row to prevent overlap', () => {
  ...
  assert.match(phoneBlock, /\.forest-brand \.brand-title\s*\{[^}]*display:\s*block/, 'phone brand text 数学小讲师联盟 should be visible under the Logo');
  assert.match(phoneBlock, /\.forest-brand\s+:global\(\.brand-logo-mark\)\s*\{[^}]*width:\s*57px[^}]*height:\s*35px/, 'phone option-B Logo should stay visible and target the child SVG globally');
